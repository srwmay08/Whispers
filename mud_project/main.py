...
def find_combat_target_in_room(player_object: player_class.Player, target_name_query: str, room_data: dict):
    ...
    for other_sid, other_player_obj in active_players.items():
        if other_sid != player_object.sid and \
           ...
           hasattr(other_player_obj, 'name') and other_player_obj.name.lower() == target_name_lower:
            return other_player_obj, "player", other_player_obj.sid, other_player_obj # Pass the player object as full_match_data
    ...
    current_monster_index_for_naming = {}
...
def finalize_character_creation(sid, player_shell: player_class.Player, game_races_data: dict, game_items_data: dict):
    ...
    if race_data:
        if not hasattr(player_shell, 'skills'):
        ...
        if not hasattr(player_shell, 'inventory'):
        ...
        if start_zone_id_from_race is not None:
            try:
                ...
                if start_zone_id_int in GAME_ROOMS:
...
def handle_player_command(data):
    ...
    try:
        ...
        if player: # Player is already logged in and active
            ...
            if hasattr(player, 'next_action_time') and time.time() < player.next_action_time:
                ...
                if not action_taken:
                    ...
                    if current_room_data and verb in current_room_data.get("exits", {}):
                        ...
                        action_taken = True
                        # ... (Look logic as before) ...
                        rt_look_cmd = config.ROUNDTIME_DEFAULTS.get('roundtime_look', 0.2)
                        if target_arg and current_room_data:
                            target_entity_data_look, entity_type_look, _, target_player_obj_look = find_combat_target_in_room(player, target_arg, current_room_data) # Ensure find_combat_target returns player obj for players
                            if target_entity_data_look: # Could be NPC, Monster, or Player object
                                look_desc_entity = ""
                                if entity_type_look == "player" and target_player_obj_look:
                                    look_desc_entity = f"You see {target_player_obj_look.name}." # Basic player look
                                    # TODO: Add more detailed player look description (race, visible equipment)
                                else: # NPC or Monster
                        ...
                        if not target_arg:
                            ...
                            if target_data_obj: # Target found (could be player object, NPC template, or Monster template)
                                if target_type == "player":
                                    defender_player_obj = target_full_match_data # This is the actual Player object of the defender
                                    if not current_room_data.get("pvp", getattr(config, "PVP_ENABLED_ROOM_TAG", False)):
                                        player.add_message("PvP is not allowed in this area.", "error_pvp")
                                        player.next_action_time = time.time() + rt_look # Short RT for failed attempt
                                    elif defender_player_obj.sid == player.sid:
                                        player.add_message("You contemplate attacking yourself, but decide against it.", "feedback_neutral")
                                        player.next_action_time = time.time() + rt_look
                                    else:
                                        if config.DEBUG_MODE: print(f"DEBUG PVP: {player.name} (SID: {player.sid}) attacking {defender_player_obj.name} (SID: {defender_player_obj.sid})")
                                        
                                        pvp_results = combat.handle_player_attack_pvp(player, defender_player_obj, GAME_ITEMS)
                                        
                                        # Send message to attacker (already handled by add_message in handle_player_attack_pvp)
                                        # if pvp_results.get("attacker_message"):
                                        #     player.add_message(pvp_results["attacker_message"]["text"], pvp_results["attacker_message"]["type"])

                                        # Send message to defender (if they are still connected)
                                        ...
                                        # Handle defender defeat (e.g., move to death room)
                                        if pvp_results.get("defender_defeated", False):
                                            player.add_message(f"You have defeated {defender_player_obj.name}!", "event_pvp_victory") # Attacker's victory message
                                            if defender_player_obj.sid in active_players: # Defender might have disconnected
                                                active_players[defender_player_obj.sid].add_message("You have been struck down in combat!", "event_pvp_defeat_major")
                                                # combat.handle_player_attack_pvp already moves the player and sets HP
                                                send_room_description(active_players[defender_player_obj.sid]) # Update defender's client with new room
                                                send_player_stats_update(active_players[defender_player_obj.sid]) # Update defender's stats
                                        
                                        # Update stats for both attacker and defender (defender's stats might have changed in handle_player_attack_pvp)
                                    ...
                                    # ... (Your existing PVE attack logic remains here) ...
                                    ...
                                    player.next_action_time = time.time() + rt_attack
                                else: # Should not happen if find_combat_target works
                                    player.add_message("You can't attack that type of target!", "error")
                                    player.next_action_time = time.time() + rt_look
                            else: # Target not found
                                player.add_message(f"You don't see '{target_arg}' to attack.", "error")
                                player.next_action_time = time.time() + rt_look
                    
                    # ... (Rest of your command handling: SEARCH, SKIN, GET, EQUIP, UNEQUIP, INVENTORY) ...
                    elif verb == "search":
                ...
                if verb_initial == "create" and name_arg_initial:
                    ...
                    if player_handler:
                        ...
                        if loaded_player:
                            ...
                            all_login_msgs = loaded_player.get_queued_messages()
            ...
            if config.DEBUG_MODE: print(f"DEBUG ERROR: SID {sid} sent command but has no active player or creation session.")
...
def game_tick_loop():
    global game_tick_counter, game_loop_active, ENTITY_COMBAT_PARTICIPANTS
    ...
    while game_loop_active:
            ...
            for entity_runtime_id in current_entities_in_combat:
                ...
                if entity_runtime_id in GAME_NPCS:
                    ...
                    try:
                        ...
                        m_room_id_str, m_template_key = parts[0], parts[1] # Assuming format roomID_templateKey_index
                        entity_data = GAME_MONSTER_TEMPLATES.get(m_template_key)
                        entity_type = "monster"
                        entity_room_id = int(m_room_id_str)
                ...
                if time.time() >= combat_state.get("next_attack_time", 0):
                    ...
                    if attack_results.get("broadcast_message"):
                         # Check if broadcast_message is a dict with text and type
                        if isinstance(attack_results["broadcast_message"], dict):
                            broadcast_to_room(player_target.current_room_id, attack_results["broadcast_message"].get("text",""), attack_results["broadcast_message"].get("type","ambient_combat"), [player_target.sid])
                        else: # Original string-only broadcast_message
        ...
        if sleep_time > 0:
            ...
            if config.DEBUG_MODE and game_tick_counter > 1:

...
try:
    ...
    class MockConfigForPlayer:
        ...
        MIN_XP_ABSORBED_PER_EVENT = 1; XP_ABSORB_LOGIC_BONUS_DIVISOR_NODE_TOWN = 5 # Corrected name
        XP_ABSORB_LOGIC_BONUS_DIVISOR_OTHER = 7 # Corrected name
        ...
        XP_ABSORB_BASE_RATE_OTHER_AREAS = 19; EQUIPMENT_SLOTS = {"torso": "Torso", "mainhand": "Main Hand"} # Simplified for mock
...
class Player:
    def __init__(self, sid, name="Unnamed Character"):
        self.sid = sid
        self.name = name
        self._queued_messages = [] # For messages to be sent to this player's client
        self.creation_phase = None # Tracks current step in character creation
        self.creation_data = {}    # Temporary data store during character creation
        self.pronouns = {}         # Stores player's chosen pronouns
        ...
        self.inventory = [] # List of item_ids
        # Initialize equipment slots based on config.EQUIPMENT_SLOTS
        ...
        self.unabsorbed_xp = 0 # XP waiting to be converted to actual XP
        
        self.factions = {} # Key: faction_id, Value: numerical standing (CONSIDER)
        self.last_mind_status = "" # For tracking changes in XP pool fullness messages
        self.next_action_time = 0  # Timestamp for when the player can next act (round time)
        self.is_in_group = False   # Flag for group status
        self.db_id = None          # MongoDB ObjectId after first save
        self.training_points = 0   # Points available for training skills/stats
    ...
    def add_message(self, text_or_payload, message_type="info"):
        """Adds a message to the player's outgoing queue."""
        if not hasattr(self, '_queued_messages') or self._queued_messages is None:
            self._queued_messages = []
        
        if isinstance(text_or_payload, dict): # If payload is already a dict (e.g. for room data)
            if 'type' not in text_or_payload and message_type: # Add type if missing
                text_or_payload['type'] = message_type
            self._queued_messages.append(text_or_payload)
        else: # If just text, create the standard message dict
            self._queued_messages.append({"text": str(text_or_payload), "type": str(message_type)})

    def get_queued_messages(self):
        """Retrieves and clears the player's outgoing message queue."""
        if not hasattr(self, '_queued_messages') or self._queued_messages is None:
            self._queued_messages = []
            return []
        messages = list(self._queued_messages) # Create a copy
        self._queued_messages.clear()          # Clear the original queue
        return messages

    def calculate_derived_stats(self, game_races_data=None, game_items_data=None):
        """Calculates max HP, MP, SP based on stats, race, level, and items."""
        effective_stats = self.stats.copy()

        if game_items_data:
            for slot_key, item_id_list in self.equipped_items.items(): # Iterate through list of items in slot
                # Ensure item_id_list is iterable, even if it's typically a single item_id or None
                ...
                for item_id in items_in_slot:
                    if item_id: # Check if item_id is not None
        ...
        # Max MP Calculation (Example: Intelligence-based)
        ...
        # Max SP Calculation (Example: Stamina-based)
        ...
        # Ensure HP/MP/SP are not negative
        self.max_hp = max(1, self.max_hp) # Minimum 1 HP
    ...
    def calculate_training_points(self, game_races_data=None):
        """Calculates training points based on level and racial bonuses."""
        # This should typically grant points *per level gained*, not just recalculate total
        # For simplicity on level up, we can just add new points.
        # If this is called at creation too, ensure it doesn't give points for level 1 unless intended.
        points_this_level = getattr(config, 'TRAINING_POINTS_PER_LEVEL', 5)
        racial_bonus_tp = 0
        if self.level == 1 and game_races_data and self.race: # Grant racial bonus at level 1
            current_race_data = game_races_data.get(self.race)
            if current_race_data:
                racial_bonus_tp = current_race_data.get("bonus_training_points_at_creation", 0) # New config in race_tables
        
        # On level up, add points_this_level. At creation (level 1), add racial_bonus_tp.
        # This assumes training_points is cumulative.
        if self.level > 1: # Gained a level
             self.training_points += points_this_level
        elif self.level == 1: # At creation
            self.training_points = racial_bonus_tp + points_this_level # Or just racial_bonus if level 1 gives 0 by default
    ...
    def get_stat_primary_bonus(self, stat_name: str, game_races_data=None):
        """Calculates the 'primary bonus' for a given stat (e.g., for HP/MP/SP calcs)."""
        ...
        if self.race and game_races_data:
            ...
            if race_data:
                # Example: race_data might have "constitution_primary_bonus_mod": 1
                racial_modifier = race_data.get(f"{stat_name}_primary_bonus_mod", 0)
        return base_stat_contribution + racial_modifier

    def get_field_xp_pool_capacity(self, game_races_data=None):
        """Calculates the player's current XP absorption pool capacity."""
        ...
        return max(100, capacity) # Ensure a minimum capacity

    def get_xp_absorption_amount_per_event(self, current_room_data=None, game_races_data=None):
        """Calculates how much XP the player absorbs this tick/event."""
        ...
        is_town = current_room_data.get("is_town", False) # Assuming rooms can be tagged as "town"
        ...
        if is_node:
            ...
            if is_super_node:
                base_rate += getattr(config, 'XP_ABSORB_SUPER_NODE_BONUS', 2)
        elif is_town: # Not a node, but in a town
            base_rate = getattr(config, 'XP_ABSORB_BASE_RATE_IN_TOWN_OFF_NODE', 22)
            logic_bonus_divisor = getattr(config, 'XP_ABSORB_LOGIC_BONUS_DIVISOR_NODE_TOWN', 5)
        else: # Other areas (wilderness, dungeon etc.)
            base_rate = getattr(config, 'XP_ABSORB_BASE_RATE_OTHER_AREAS', 19)

        logic_primary_bonus = self.get_stat_primary_bonus("logic", game_races_data)
        logic_absorb_contribution = logic_primary_bonus // logic_bonus_divisor if logic_bonus_divisor > 0 else 0

        pool_bonus = 0
        unabsorbed = getattr(self, 'unabsorbed_xp', 0)
        ...
        total_absorb = base_rate + logic_absorb_contribution + pool_bonus + group_bonus
    ...
    def get_current_mind_status(self, game_races_data=None):
        """Determines the player's 'mind status' based on unabsorbed XP."""
        ...
        if capacity <= 0: # Avoid division by zero
            return "Clear as a bell" if current_unabsorbed_xp == 0 else "Muddled" 
        if current_unabsorbed_xp == 0:
            return "Clear as a bell" # Special phrase for exactly 0 unabsorbed XP.

        ratio = current_unabsorbed_xp / capacity
        
        # Ensure thresholds are sorted from highest ratio to lowest for correct matching
        ...
        for status_info in thresholds:
            # Handle the special "Clear as a bell" case for exactly 0 XP separately (done above)
            if status_info['phrase'] == "Clear as a bell":
                continue 
            if ratio >= status_info['threshold']:
                return status_info['phrase']
        
        return "Fresh and clear" # Default if ratio is low but not 0, and no other threshold met.

    def _check_and_send_mind_status(self, game_races_data=None, force_send=False):
        """Checks if mind status changed and sends an update to the player."""
    ...
    def add_xp_to_pool(self, amount: int, game_races_data=None):
        """Adds XP to the player's unabsorbed pool."""
        if amount <= 0: return

        capacity = self.get_field_xp_pool_capacity(game_races_data)
        current_unabsorbed = getattr(self, 'unabsorbed_xp', 0)
        space_available = capacity - current_unabsorbed
        amount_to_add = min(amount, max(0, space_available)) # Cannot add more than available space
        ...
        if amount_to_add < amount and amount > 0 : # Tried to add more than capacity allowed
    ...
    def get_faction_standing(self, faction_id: str) -> int:
        """Returns the player's numerical standing with a given faction."""
        return getattr(self, 'factions', {}).get(faction_id, 0) # Default to 0 (neutral) if no specific standing

    def get_faction_display_str(self, faction_display_names_config: dict) -> str:
        """Gets a formatted string of player's faction standings."""
        ...
        for faction_id, standing in sorted(self.factions.items()): # Sort for consistent display
            display_name = faction_display_names_config.get(faction_id, faction_id.replace("_", " ").title())
            
            standing_desc = "" # Descriptive tier
            ...
            # Add more tiers as needed in config.py
    ...
    def update_faction(self, faction_id: str, amount_change: int):
        """Updates the player's standing with a faction."""
        if not hasattr(self, 'factions') or not isinstance(self.factions, dict):
            self.factions = {}
        
        current_standing = self.factions.get(faction_id, 0)
        new_standing = current_standing + amount_change
        
        # Clamp to min/max defined in config
        ...
        if amount_change < 0:
            self.add_message(f"Your standing with {display_faction_name} has worsened.", "faction_update_negative")
        elif amount_change > 0:
            self.add_message(f"Your standing with {display_faction_name} has improved.", "faction_update_positive")
        
        if config.DEBUG_MODE:
            print(f"DEBUG PLAYER ({self.name}): Faction '{faction_id}' changed by {amount_change}. New standing: {new_standing}.")
    ...
    def equip_item(self, item_id_to_equip: str, target_slot_key: str, game_items_data: dict, game_races_data=None):
        ...
        if not item_allowed_slots or target_slot_key not in item_allowed_slots: # Check if item can go in target slot
            self.add_message(f"The {item_template.get('name', 'item')} cannot be equipped to {actual_slot_name_for_msg}.", "error")
            return False

        # Handle un-equipping item currently in the target slot
        currently_equipped_id = self.equipped_items.get(target_slot_key)
        if currently_equipped_id:
            if not self.unequip_item(target_slot_key, game_items_data, game_races_data, silent=True): # Try to unequip silently
                self.add_message(f"Cannot unequip {game_items_data.get(currently_equipped_id,{}).get('name','item')} to make space.", "error")
                return False # Failed to make space

        # Handle two-handed weapons needing to clear the other hand
        ...
        if is_two_handed_weapon:
            ...
            if self.equipped_items.get(other_hand_slot):
                ...
                if not self.unequip_item(other_hand_slot, game_items_data, game_races_data, silent=True):
                    ...
                    return False # Cannot clear other hand

        # Equip the new item
        self.equipped_items[target_slot_key] = item_id_to_equip
        if is_two_handed_weapon: # Also mark the other hand slot as occupied by this item if it's 2H
             other_hand_slot_for_2h = "offhand" if target_slot_key == "mainhand" else "mainhand"
             self.equipped_items[other_hand_slot_for_2h] = item_id_to_equip # Mark as occupied by the same item
        ...
        self.calculate_derived_stats(game_races_data, game_items_data) # Recalculate stats
        return True
    ...
    def unequip_item(self, slot_key_to_unequip: str, game_items_data: dict, game_races_data=None, silent=False):
        ...
        if not item_id_unequipped: # Nothing in that slot
        ...
        # If it was a two-handed weapon, clear the conceptual link in the other hand slot
        if item_template and ("twohand" in item_template.get("slot", []) or item_template.get("is_two_handed", False)):
            ...
            if self.equipped_items.get(other_hand_slot) == item_id_unequipped: # If other hand was holding same 2H item
                self.equipped_items[other_hand_slot] = None

        if not silent:
            self.add_message(f"You unequip the {item_name_for_msg} from your {actual_slot_name_for_msg}.", "feedback_unequip")
        
        self.calculate_derived_stats(game_races_data, game_items_data) # Recalculate stats
        return True

    def get_equipped_item_data(self, slot_key: str, game_items_data: dict):
        """Gets the item template data for an item in a given slot."""
    ...
    def get_armor_type(self, game_items_data: dict) -> str:
        """Determines the player's current armor type based on torso armor."""
        ...
        return config.DEFAULT_UNARMORED_TYPE # Default if no torso armor or not armor type

    def to_dict(self):
        """Serializes player data to a dictionary for saving."""
        return {
            "sid": self.sid, "name": self.name, "race": self.race, "pronouns": self.pronouns,
            "stats": self.stats, "skills": self.skills, "inventory": self.inventory,
            "equipped_items": self.equipped_items, "current_room_id": self.current_room_id,
            "hp": self.hp, "max_hp": self.max_hp, "mp": self.mp, "max_mp": self.max_mp,
            "sp": self.sp, "max_sp": self.max_sp, "level": self.level, "xp": self.xp,
            "creation_phase": self.creation_phase, # Persist creation phase if not complete
            "unabsorbed_xp": getattr(self, 'unabsorbed_xp', 0),
            "factions": getattr(self, 'factions', {}),
            "last_mind_status": getattr(self, 'last_mind_status', ""),
            "is_in_group": getattr(self, 'is_in_group', False),
            "training_points": getattr(self, 'training_points', 0),
            "db_id": str(self.db_id) if self.db_id else None # Store db_id as string
        }

    @classmethod
    def from_dict(cls, data, sid_on_load=None, game_races_data=None, game_items_data=None):
        """Deserializes player data from a dictionary to a Player object."""
        # Use sid_on_load if provided (for active connections), else use stored sid (for offline loading/utils)
        ...
        # Load MongoDB _id if present
        db_id_str = data.get("db_id", data.get("_id")) # Check both db_id and _id (legacy)
        if db_id_str:
            from bson import ObjectId # Import here to avoid circular dependency if Player is imported elsewhere
            try:
                player.db_id = ObjectId(db_id_str)
            except Exception: # bson.errors.InvalidId or similar
                if config.DEBUG_MODE: print(f"DEBUG PLAYER LOAD: Invalid db_id format '{db_id_str}' for {player.name}. Will be set on next save.")
                player.db_id = None # Will be set on next save if it was invalid

        player.race = data.get("race")
        player.pronouns = data.get("pronouns", {})
        
        loaded_stats = data.get("stats", {})
        for stat_key in config.ALL_STATS_ORDERED: # Ensure all stats are initialized
            player.stats[stat_key] = loaded_stats.get(stat_key, config.DEFAULT_STAT_VALUE)
            
        player.skills = data.get("skills", getattr(config, 'INITIAL_SKILLS', {}).copy())
        player.inventory = data.get("inventory", [])
        
        loaded_equipped = data.get("equipped_items", {})
        # Ensure all defined slots exist on the player object, even if not in saved data
        for slot_key_cfg in config.EQUIPMENT_SLOTS.keys():
            player.equipped_items[slot_key_cfg] = loaded_equipped.get(slot_key_cfg) # Will be None if not in loaded_equipped

        player.current_room_id = data.get("current_room_id", config.DEFAULT_START_ROOM_ID)
        
        # Load HP/MP/SP but they will be validated against recalculated max values
        player.max_hp = data.get("max_hp", 10) # Initial load, will be recalculated
        player.hp = data.get("hp", player.max_hp)
        player.max_mp = data.get("max_mp", 0)
        player.mp = data.get("mp", player.max_mp)
        player.max_sp = data.get("max_sp", 0)
        ...
        player.creation_phase = data.get("creation_phase") # Restore creation phase
        ...
        # This ensures max HP/MP/SP are correct based on current stats, race, items.
        player.calculate_derived_stats(game_races_data, game_items_data)
        
        # Validate current HP/MP/SP against newly calculated maximums
        player.hp = min(player.hp, player.max_hp)
        player.mp = min(player.mp, player.max_mp)
        player.sp = min(player.sp, player.max_sp)
        # Ensure HP is at least 1 if max_hp is > 0, unless specific death mechanics are in place
        if player.max_hp > 0 and player.hp <= 0 and player.level > 0 : # Assuming level 0 might be a special state
             player.hp = 1 # Or move to a death state/room
    ...
    def get_client_data(self, game_races_data=None, game_items_data=None):
        """Gets a summary of player data suitable for sending to the client's UI."""
        ...
        if game_races_data and self.race:
            ...
            if race_data_entry:
                race_display_name = race_data_entry.get("name", self.race.title().replace("_", " "))
        ...
        if game_items_data:
            for slot_key, item_id in self.equipped_items.items():
                ...
                if item_id:
                    ...
                    equipped_display[slot_display_name] = "---" # More explicit "empty" display
        
        return {
            "name": self.name,
            "race_key": self.race, # Send the key for potential client-side logic
            "race_display_name": race_display_name,
            "hp": self.hp, "max_hp": self.max_hp,
            "mp": self.mp, "max_mp": self.max_mp,
            "sp": self.sp, "max_sp": self.max_sp,
            "level": self.level, "xp": self.xp,
            "unabsorbed_xp": getattr(self, 'unabsorbed_xp', 0),
            "stats": self.stats, # Could be sent for a character sheet display
            "inventory_count": len(self.inventory),
            "equipped_items_display": equipped_display, # For a simple equipment view
            "current_room_id": self.current_room_id, # For client-side debugging or context
            "mind_status": self.get_current_mind_status(game_races_data),
            "gold": getattr(self, 'gold', 0) # Send gold
        }
